...
class EmotionChatMachine(object):
    def __init__(self, config_file, session, words2idx, class_weights, word_embeddings, generic_word_length, start_id, end_id,
                 name=""):
        ...
        self.n_words_y = tf.placeholder(shape=[1], dtype=tf.float32,
                                        name=self.name + "_n_words_y")
        ...
        self.saver = tf.train.Saver(max_to_keep=self.config.epochs_to_train)
    ...
    def compute_loss(self):
        ...
        # cross_entropy_2=tf.losses.sparse_softmax_cross_entropy(self.predict_y, total_sfx_scores)

        # specify the weights for each sample in the batch (without having to compute the onehot label matrix)
        labels=tf.reshape(self.predict_y,[self.batch_size*self.config.max_len])
        logits=tf.reshape(total_sfx_scores,[self.batch_size*self.config.max_len,self.num_total_word])
        weights = tf.gather(self.class_weights, labels)

        # compute the loss
        loss_likelihood=tf.losses.sparse_softmax_cross_entropy(labels,logits,weights,reduction=tf.losses.Reduction.SUM)
        loss_likelihood=loss_likelihood / self.batch_size
    ...
    def train_step(self, this_input_x, this_post_len, this_input_y, this_predict_y, n_words_y, this_emotion_labels,
                   this_emotion_mask):
    ...
    def valid_step(self, this_input_x, this_post_len, this_input_y, this_predict_y, n_words_y, this_emotion_labels,
                   this_emotion_mask):
        input_feed = {self.input_x: this_input_x,
                      self.input_length: this_post_len,
                      self.input_y: this_input_y,
        ...
        output_feed = [self.entropy_loss, self.reg_loss, self.total_loss]
        results = self.session.run(output_feed, input_feed)
        entropy_loss, reg_loss, total_loss = results
        return entropy_loss, reg_loss, total_loss

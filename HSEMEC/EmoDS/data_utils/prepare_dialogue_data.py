    ...
    for i in range(6):
        ...
        while length < emotion_word_num:
            if sorted_emotion_words[index][0] not in total_emotion_word:
                ...
                length += 1
            index += 1
            # if index>=len(emotion_word_lists) and length < emotion_word_num:
            #     print(f'{id2emotion[i]} <200. =',length)
            #     for word in sorted_emotion_words:
            #         total_emotion_word.append(word[0])
            #         length += 1
            #         if length >= emotion_word_num:
            #             break
    assert 6 * emotion_word_num == len(total_emotion_word)
...
def write_test_data(test_data, write_file, id2words, test_emotion_labels):
    ...
    for data, label in zip(test_data, test_emotion_labels):
        words = [id2words[index] for index in data[0]]
        sentence = " ".join(words)
        emo = id2emotion[label]
        if emo == 'none':
            emo = 'other'
        final_sentence = sentence + '\t' + emo

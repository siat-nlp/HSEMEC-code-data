...
def save_2Dnumpy_as_list_to_txt(f, vecs, type='src'):
    outdim = len(vecs)
    if 0 == len(vecs):
        return [len(vecs), 0]
...
def save_3Dnumpy_as_list_to_txt(f, vecs):
    outdim = len(vecs)
    if 0 == len(vecs):
        return [len(vecs), 0, 0]
    middim = len(vecs[0])
    indim = len(vecs[0][0])
    print(str(outdim) + "\t" + str(middim) + '\t' + str(indim) + "\n")
    f.write(str(outdim) + "\t" + str(middim) + '\t' + str(indim) + "\n")
    for vec in vecs:
        for v in vec:
            f.write(" ".join(list(map(str, v.tolist()))) + "\n")
